...
export const sendSMS = async (phoneNumber: string, message: string, simId?: string): Promise<boolean> => {
  return new Promise((resolve) => {
    // Simulate network delay
    setTimeout(() => {
      try {
        console.log(`Sending SMS to ${phoneNumber} via SIM ${simId || 'default'}: ${message}`);
...
export const getSIMInfo = async (): Promise<{carrier: string, country: string, phoneNumber: string, simId: string}[]> => {
...
export const handleSMSStatusUpdate = (messageId: string, status: 'pending' | 'sent' | 'failed' | 'received') => {
...
export const saveReceivedSMS = (phoneNumber: string, message: string) => {
  const receivedMessages = JSON.parse(storage.getString('received_messages') || '[]');
  const newMessage = {
