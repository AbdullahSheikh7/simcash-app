...
export default function MessagesScreen() {
  const [filter, setFilter] = useState<'all' | 'pending' | 'sent' | 'failed' | 'received'>('all');
  ...
  return (
    <ScrollView style={styles.container}>
      <ThemedView style={styles.header}>
        <ThemedText type="title" style={styles.title}>Message Control</ThemedText>
      ...
      <ThemedView style={styles.filterContainer}>
        ...
        >
          <ThemedText style={[styles.filterText, filter === 'sent' && styles.activeFilterText]}>
            Sent
          </ThemedText>
        </TouchableOpacity>
        <TouchableOpacity 
        ...
        >
          <ThemedText style={[styles.filterText, filter === 'failed' && styles.activeFilterText]}>
            Failed
          </ThemedText>
        </TouchableOpacity>
        <TouchableOpacity 
        ...
        >
          <ThemedText style={[styles.filterText, filter === 'received' && styles.activeFilterText]}>
            Received
          </ThemedText>
        </TouchableOpacity>
      </ThemedView>
        ...
        {filteredMessages.length > 0 ? (
          filteredMessages.map(message => (
            <MessageItem
              ...
              onPress={() => message.status === 'failed' && handleResendMessage(message.id)}
            />
          ))
        ) : (
          <ThemedView style={styles.emptyContainer}>
            <ThemedText>No {filter === 'all' ? '' : filter + ' '}messages yet</ThemedText>
          </ThemedView>
        )}
      </ThemedView>
    </ScrollView>
